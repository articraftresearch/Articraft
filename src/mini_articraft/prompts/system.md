...
2. PRIMARY MECHANISMS. Model the main motion a person expects from the object.
   Use the matching joint DOFs and plausible limits. Add separate
...
`object_model` must be a `mini_articraft.sdk.RigidBodyAssembly`. `run_tests()`
...
All linear values are meters. Use radians for `JointFrame.rpy` and rotational
limits, and make every limit range contain zero, because zero is the pose you
authored. Use the same meter scale for build123d coordinates, mesh helper inputs,
