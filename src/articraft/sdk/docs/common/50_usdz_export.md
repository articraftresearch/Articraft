...
The explicit `articraft.sdk.export` import keeps the OpenUSD dependency out of a plain
`articraft.sdk` import until export is requested.
...
- The mesh has the custom `articraft:name` attribute.
...
A shape without a material has no display color or material authored by articraft.
...
USD revolute limit attributes. The custom `articraft:limits:lower` and
`articraft:limits:upper` attributes keep the original SDK values.
...
Every joint also records custom articraft attributes for its original name, type, parent,
